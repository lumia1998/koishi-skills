...
**大文件处理 (分卷及体积压缩限制)**：根据总页数智能限制单文件体积上限（比例为每 50 页限 10MB）。首先尝试打包并发送单文件，仅在文件大小超标时才触发分卷分段打包发送：
- **≤ 50 页**：单 ZIP 限制 10MB。若超 10MB 则自动分卷发送。
- **50 ~ 100 页（如 90 页）**：单 ZIP 限制 20MB。若单 ZIP 体积 < 20MB 则直接作为单文件发送；超过 20MB 则分卷打包发送。
- **200 ~ 250 页（如 203 页）**：单 ZIP 限制 50MB。若超 50MB 则分卷发送。
...
- **单文件体积限制规则**：单 ZIP 文件大小限制为 `10 * math.ceil(total_pages / 50)` MB。
- **自适应分卷逻辑**：
  - 首先尝试直接打包为单个 ZIP 文件。
  - 如果单文件体积未超限（如：90 页文件大小为 15MB，小于 20MB 上限），则**直接发送单个 ZIP 文件**，不进行拆分。
  - 如果单文件体积超限（如：90 页文件大小为 23MB，大于 20MB 上限），则**自动分割**为多份分卷（例如分拆成每份约 45 页的两份 ZIP）打包发送，使每个分卷的大小均降到安全体积内。
- **命名规范**：单文件打包为 `标题_第1话.zip`；分卷打包时命名为 `标题_第1话_part01.zip`、`标题_第1话_part02.zip`... 密码相同。
