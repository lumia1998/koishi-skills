---
name: comic
description: 聚合漫画 skill，同时覆盖禁漫天堂(JM)和哔咔漫画(Bika)。通过本地 comic-api 服务提供搜索、下载、排行榜、分类浏览、随机推荐等功能。触发词：我要看xxx的本子、高岛老师的本子、花火的本子、来个崩铁同人、给我看xxx、推荐本子、jmxxxxxx、禁漫xxxxxx、哔咔xxxxxx、排行榜、来个随机本子、全彩/同人/人妻/单行本等分类请求。若用户提供 jmxxxxxx 格式的 JM 号且数字>100，直接下载；否则先搜索返回列表再由用户选择。
---
...
**大文件处理**：章节超过 50 页时，自动分 50 页一组拆分为多个 ZIP 文件（约每份 10MB）。
...
用户说「高岛老师的 ba 本子」或「崩铁花火的本子」时：

1. **先用模型知识**识别：「高岛老师」= 高岛礼子？「ba」= 碧蓝档案？「花火」= 崩坏：星穹铁道角色
2. **若不确定**，联网搜索补充知识（如 `高岛 ba 漫画家`）
3. **构造关键词**调用搜索：`花火 崩铁`、`高岛 碧蓝档案`
4. 返回列表让用户确认
